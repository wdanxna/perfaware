...
const std::string filepath = "../computer_enhance/perfaware/part1/listing_0038_many_register_mov";

std::string get_register(uint8_t reg, uint8_t w) {
    static const std::string tb[2*8] = {
...
std::string immediate_to_memory(char op, std::ifstream &s)
{
    ...
    uint8_t mod = (b2 & 0b11000000) >> 6;
    uint8_t rm = (b2 & 0b000000111);
    //get destination address first, since the displacement located at next 2 bytes
...
std::string decode(std::ifstream& s) {
    char op{};
    if (!s.read(&op, sizeof(char))) return "";
    return decode(op, s);
}
